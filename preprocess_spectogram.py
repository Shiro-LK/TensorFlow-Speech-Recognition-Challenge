...
def data_augmentation(data, proba=0.5, coeff_noise=0.25, coeff_time=4000):
    if np.random.uniform(0.0, 1.0) <= proba:
        ...
        #print(data.dtype)
    return data
def log_spectrogram(path, window_size=20, step_size=10, eps=1e-10, data_aug=False, proba_data_aug=0.5, coeff_noise=0.3, coeff_time=4000):
    '''
    ...
    # get the specgram
    freq, times, specgram = signal.spectrogram(d, fs= sample_rate ,window='hann', nperseg=nperseg, noverlap=noverlap, detrend=False)
...
def load_data_with_spectrogram(paths, transpose=False, p='', window_size=20, step_size=10, eps=1e-10, data_aug=False, proba_data_aug=0.5, coeff_noise=0.3, coeff_time=4000):
    '''
        return array of spectrogram (number, freq, times) by default from list of paths
    '''
    #print(data_aug)
    if transpose==False:
        data = np.asarray([ log_spectrogram(p+path, window_size=window_size, step_size=step_size, eps=eps, data_aug=data_aug,  proba_data_aug=proba_data_aug, coeff_noise=coeff_noise, coeff_time=coeff_time)[2] for path in paths]) # freq, windows
    else:
        data = np.asarray([ log_spectrogram(p+path, window_size=window_size, step_size=step_size, eps=eps, data_aug=data_aug,  proba_data_aug=proba_data_aug, coeff_noise=coeff_noise, coeff_time=coeff_time)[2].T for path in paths]) # windows, freq
    #print('data : ', data.shape)
    return data
...
def main2():
    data, labels = load_data_with_spectrogram('data_labelised2_test09v2.txt', True)
#main2()
#main()
